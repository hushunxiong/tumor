/**
 * 
 */
package com.wonders.health.tumor.follow.service;

import com.wonders.health.tumor.follow.dao.ScFollowDao;
import com.wonders.health.tumor.follow.entity.ScFollow;
import com.wonders.health.tumor.common.model.AjaxReturn;
import com.wonders.health.tumor.common.model.BaseEntity;
import com.wonders.health.tumor.common.utils.IdGen;
import org.apache.commons.lang3.StringUtils;
import org.springframework.beans.BeanUtils;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;
import java.util.List;
import java.util.Map;

import com.wonders.health.tumor.common.model.DataGrid;
import com.wonders.health.tumor.common.model.DataGridSearch;

/**
 * 胃癌随访Service
 * @author zhangyi
 */
@Service
@Transactional(readOnly = true)
public class ScFollowService {

    @Autowired
    private ScFollowDao scFollowDao;

	public DataGrid<ScFollow> findPage(DataGridSearch search) {
        Integer count = scFollowDao.pageCount(search);
        List<ScFollow> list = null;
        if (count > 0) {
        	list = scFollowDao.pageList(search);
        }
        return new DataGrid<ScFollow>(count,list);
    }

    public ScFollow findById(String id) {
        return scFollowDao.get(id);
    }

    @Transactional(readOnly = false)
    public AjaxReturn<Map<String, String>> saveOrUpdate(ScFollow vo, String userId) {
        if (vo != null && StringUtils.isNotBlank(vo.getId())) { //修改
            ScFollow po = scFollowDao.get(vo.getId());
            if (po != null) {
                BeanUtils.copyProperties(vo, po, BaseEntity.IGNORES);
                po.initByUpdate(userId);
                scFollowDao.update(po);
                return new AjaxReturn<Map<String, String>>(true, "修改成功");
            } else {
                return new AjaxReturn<Map<String, String>>(false, "传入ID无法找到记录");
            }
        } else { //新增
            vo.setId(IdGen.uuid());
            vo.init(userId);
            scFollowDao.insert(vo);
            return new AjaxReturn<Map<String, String>>(true, "保存成功");
        }
    }

    @Transactional(readOnly = false)
    public void deleteById(String id) {
        scFollowDao.delete(id);
    }

}